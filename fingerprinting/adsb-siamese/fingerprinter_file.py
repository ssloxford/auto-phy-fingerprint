...
os.environ[
    'TF_CPP_MIN_LOG_LEVEL'] = '1'  # 0 = all messages are logged (default behavior), 1 = INFO messages are not printed, 2 = INFO and WARNING messages are not printed, 3 = INFO, WARNING, and ERROR messages are not printed
...
if len(sys.argv) < 2:
    h5siamese_filename = "/data/adsb-rhb-apr2021.hdf5"
...
t0 = time.time()
msg_df = fingerprint_model.predict(test_in)  # Fingerprint all the messages
t1 = time.time()
print(f"{t1 - t0}s")

fp_list = []  # per message fingerprints
msgi_list = []  # message ids
icaos_list = []
    ...
    if claimedicao not in known_aircraft:
        ...
        if fp_method is 'centroidN':
            if fp_n == 1:
                ...
                fp_dict[claimedicao] = [msg_df[msgi]]
            elif len(fp_dict[claimedicao]) < fp_n - 1:
                fp_dict[claimedicao].append(msg_df[msgi])
            else:
                fp_dict[claimedicao].append(msg_df[msgi])
            ...
            if fp_n == 1:
                ...
                for i, m in enumerate(fp_dict[claimedicao]):
                    ...
                    tfp_list.append(fp_result)
                max_ = max(tfp_list)
                fp = fp_dict[claimedicao][tfp_list.index(max_)]
                del fp_dict[claimedicao]
                save_fp = True
        ...
        # otherwise, build fingerprint array
        fp_list.append(known_aircraft[claimedicao])
        msgi_list.append(msgi)
        icaos_list.append(claimedicao)

fp_df = np.array(fp_list)
msg_df_nofp = msg_df[np.array(msgi_list)]

# for each message, compare it to the fingerprint
t0 = time.time()
if fp_method is 'first':
    compare_result = verification_model.predict([msg_df_nofp, fp_df])
    matches = compare_result.flatten() > 0.5
elif fp_method is 'bestN':
    compare_result = verification_model.predict([msg_df_nofp, fp_df])
    matches = compare_result.flatten() > 0.5
    ...
    raise ValueError('Invalid fingerprinting method given')
results[True] = matches.sum()
results[False] = matches.shape[0] - results[True]
print(results, results[True] / (results[True] + results[False]))
for (match, claimedicao) in zip(matches, icaos_list):
