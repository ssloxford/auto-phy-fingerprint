...
class FingerprintDB:
	...
	def get_all(self):
		return self.session.query(dbFingerprintMessage).all()
...
class FirstMsgFingerprinter:
	...
	def fingerprint_msg(self, msg):
		known = self.known_aircraft.get_all()
		similarities = {}

		#TODO: put them in one array and do a single predict call
		for candidate in known:
			ref = self.known_aircraft.get(candidate.icao)
			#ref = candidate.fp_msg
			...
			similarities[candidate.icao] = compare_result.flatten()[0]
